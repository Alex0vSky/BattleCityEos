// Copyright 2025 Alex0vSky (https://github.com/Alex0vSky), Copyright 2015-2021 (https://github.com/KrystianKaluzny/Tanks)
#include "game.h"
...
Game::Game(std::vector<Player *> players, int previous_level)
{
    ...
    {
        player->clearFlag(TankStateFlag::TSF_MENU);
        player->lives_count++;
        player->respawn();
    }
    m_pause = false;
    ...
    else
    {
        ...
        for(auto &player : m_players)
            for(auto &enemy : m_enemies)
            {
                ...
                //checking the collision of the player's projectiles with the opponent
        ...
        //checking collision between player and bunus
        for(auto player : m_players)
            for(auto bonus : m_bonuses)
                checkCollisionPlayerWithBonus(player, bonus);

        //Checking tanks for collision with the level
...
void Game::nextLevel()
{
	...
	Engine::getEngine( ).getAudio( ) ->playSound( ) ->gamestart( );
}
...
void Game::generateEnemy()
{
    ...
    p = static_cast<float>(rand()) / RAND_MAX;
    if(p < 0.12) e->setFlag(TankStateFlag::TSF_BONUS);
