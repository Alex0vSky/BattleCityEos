// src\schema\ice\objects\tank.cpp // Copyright 2025 Alex0vSky (https://github.com/Alex0vSky), Copyright 2015-2021 (https://github.com/KrystianKaluzny/Tanks)
#include "tank.h"
#include "appconfig.h"

Tank::Tank()
    : BaseObject(AppConfig::enemy_starting_point.at(0).x, AppConfig::enemy_starting_point.at(0).y, sprite_t::ST_TANK_A)
{
...
void Tank::update(Uint32 dt)
{
    ...
    if(m_sprite->frames_count > 1 && (testFlag(TankStateFlag::TSF_LIFE) ? speed > 0 : true)) //no animation if the tank is not trying to move
    {
        ...
        if(m_frame_display_time > (testFlag(TankStateFlag::TSF_MENU)  ? m_sprite->frame_duration / 2 : m_sprite->frame_duration))
        {
...
rect_t Tank::nextCollisionRect(Uint32 dt)
{
...
void Tank::collide(rect_t &intersect_rect)
{
...
void Tank::setFlag(TankStateFlag flag)
{
    ...
    {
        if(m_shield == nullptr) m_shield = new BaseObject(pos_x, pos_y, sprite_t::ST_SHIELD);
         m_shield_time = 0;
    }
    if(flag == TankStateFlag::TSF_BOAT)
    {
         if(m_boat == nullptr) m_boat = new BaseObject(pos_x, pos_y, type == sprite_t::ST_PLAYER_1 ? sprite_t::ST_BOAT_P1 : sprite_t::ST_BOAT_P2);
    }
