...
Tank::Tank()
    : BaseObject(AppConfig::enemy_starting_point.at(0).x, AppConfig::enemy_starting_point.at(0).y, ST_TANK_A)
{
...
void Tank::setFlag(TankStateFlag flag)
{
    ...
    {
        if(m_shield == nullptr) m_shield = new BaseObject(pos_x, pos_y, ST_SHIELD);
         m_shield_time = 0;
    }
    if(flag == TankStateFlag::TSF_BOAT)
    {
         if(m_boat == nullptr) m_boat = new BaseObject(pos_x, pos_y, type == ST_PLAYER_1 ? ST_BOAT_P1 : ST_BOAT_P2);
    }
